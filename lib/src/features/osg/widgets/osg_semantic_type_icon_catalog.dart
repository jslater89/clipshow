import "package:flutter/material.dart";

/// Curated Material icons for semantic types (label used for search only).
///
/// Annotated for icon tree-shaking: only these code points are bundled in
/// release builds when resolved via [osgMaterialIconFromCodePoint].
@staticIconProvider
abstract final class OsgSemanticTypeIcons {
  const OsgSemanticTypeIcons._();

  static const List<(String, IconData)> choices = <(String, IconData)>[
    ("Label", Icons.label_outline),
    ("Tag", Icons.local_offer_outlined),
    ("Star", Icons.star_outline),
    ("Person", Icons.person_outline),
    ("Group", Icons.groups_outlined),
    ("Mic", Icons.mic_outlined),
    ("Music", Icons.music_note_outlined),
    ("Video", Icons.videocam_outlined),
    ("Movie", Icons.movie_outlined),
    ("Sports", Icons.sports_soccer_outlined),
    ("Game", Icons.sports_esports_outlined),
    ("Trophy", Icons.emoji_events_outlined),
    ("Timer", Icons.timer_outlined),
    ("Schedule", Icons.schedule_outlined),
    ("Calendar", Icons.calendar_today_outlined),
    ("Location", Icons.location_on_outlined),
    ("Map", Icons.map_outlined),
    ("Camera", Icons.photo_camera_outlined),
    ("Image", Icons.image_outlined),
    ("Palette", Icons.palette_outlined),
    ("Brush", Icons.brush_outlined),
    ("Text", Icons.text_fields_outlined),
    ("Title", Icons.title_outlined),
    ("Chat", Icons.chat_bubble_outline),
    ("Forum", Icons.forum_outlined),
    ("Announcement", Icons.campaign_outlined),
    ("News", Icons.newspaper_outlined),
    ("Article", Icons.article_outlined),
    ("Bookmark", Icons.bookmark_outline),
    ("Folder", Icons.folder_outlined),
    ("Attach", Icons.attach_file),
    ("Link", Icons.link_outlined),
    ("Cloud", Icons.cloud_outlined),
    ("Download", Icons.download_outlined),
    ("Upload", Icons.upload_outlined),
    ("Share", Icons.share_outlined),
    ("Favorite", Icons.favorite_outline),
    ("Heart", Icons.favorite_border),
    ("Flag", Icons.flag_outlined),
    ("Warning", Icons.warning_amber_outlined),
    ("Error", Icons.error_outline),
    ("Info", Icons.info_outline),
    ("Help", Icons.help_outline),
    ("Settings", Icons.settings_outlined),
    ("Tune", Icons.tune_outlined),
    ("Build", Icons.build_outlined),
    ("Code", Icons.code_outlined),
    ("Bug", Icons.bug_report_outlined),
    ("Science", Icons.science_outlined),
    ("School", Icons.school_outlined),
    ("Work", Icons.work_outline),
    ("Business", Icons.business_outlined),
    ("Payment", Icons.payment_outlined),
    ("Shopping", Icons.shopping_cart_outlined),
    ("Local", Icons.local_activity_outlined),
    ("Flight", Icons.flight_outlined),
    ("Hotel", Icons.hotel_outlined),
    ("Restaurant", Icons.restaurant_outlined),
    ("Coffee", Icons.local_cafe_outlined),
    ("Car", Icons.directions_car_outlined),
    ("Bike", Icons.pedal_bike_outlined),
    ("Home", Icons.home_outlined),
    ("Apartment", Icons.apartment_outlined),
    ("Pets", Icons.pets_outlined),
    ("Nature", Icons.park_outlined),
    ("Weather", Icons.wb_sunny_outlined),
    ("Water", Icons.water_drop_outlined),
    ("Fire", Icons.local_fire_department_outlined),
    ("Bolt", Icons.bolt_outlined),
    ("Flash", Icons.flash_on_outlined),
    ("Visibility", Icons.visibility_outlined),
    ("Search", Icons.search),
    ("Filter", Icons.filter_list_outlined),
    ("Sort", Icons.sort_outlined),
    ("List", Icons.list_outlined),
    ("Grid", Icons.grid_view_outlined),
    ("Dashboard", Icons.dashboard_outlined),
    ("Analytics", Icons.analytics_outlined),
    ("Bar chart", Icons.bar_chart_outlined),
    ("Pie chart", Icons.pie_chart_outline),
    ("Numbers", Icons.numbers_outlined),
    ("Abc", Icons.abc_outlined),
    ("Translate", Icons.translate_outlined),
    ("Language", Icons.language_outlined),
    ("Public", Icons.public_outlined),
    ("Lock", Icons.lock_outline),
    ("Shield", Icons.shield_outlined),
    ("Verified", Icons.verified_outlined),
    ("Check", Icons.check_circle_outline),
    ("Close", Icons.cancel_outlined),
    ("Add", Icons.add_circle_outline),
    ("Remove", Icons.remove_circle_outline),
    ("Edit", Icons.edit_outlined),
    ("Delete", Icons.delete_outline),
    ("Archive", Icons.archive_outlined),
    ("Inbox", Icons.inbox_outlined),
    ("Send", Icons.send_outlined),
    ("Mail", Icons.mail_outline),
    ("Phone", Icons.phone_outlined),
    ("Headset", Icons.headset_mic_outlined),
    ("Volume", Icons.volume_up_outlined),
    ("Play", Icons.play_circle_outline),
    ("Pause", Icons.pause_circle_outline),
    ("Stop", Icons.stop_circle_outlined),
    ("Skip", Icons.skip_next_outlined),
    ("Replay", Icons.replay_outlined),
    ("Loop", Icons.loop_outlined),
    ("Shuffle", Icons.shuffle_outlined),
    ("Queue", Icons.queue_music_outlined),
    ("Album", Icons.album_outlined),
    ("Radio", Icons.radio_outlined),
    ("Podcast", Icons.podcasts_outlined),
    ("Live tv", Icons.live_tv_outlined),
    ("Theaters", Icons.theaters_outlined),
    ("Emoji", Icons.emoji_emotions_outlined),
    ("Celebration", Icons.celebration_outlined),
    ("Gift", Icons.card_giftcard_outlined),
    ("Cake", Icons.cake_outlined),
    ("Interests", Icons.interests_outlined),
  ];
}

const List<(String, IconData)> osgSemanticTypeIconChoices =
    OsgSemanticTypeIcons.choices;

final Map<int, IconData> osgSemanticTypeIconByCodePoint =
    <int, IconData>{
      for (final (_, IconData icon) in OsgSemanticTypeIcons.choices)
        icon.codePoint: icon,
    };

/// Resolves a persisted code point to a const [IconData] from the catalog.
///
/// Unknown code points fall back to [Icons.label_outline] so release builds
/// never construct non-const [IconData] instances (which disables tree-shaking).
IconData osgMaterialIconFromCodePoint(int codePoint) {
  return osgSemanticTypeIconByCodePoint[codePoint] ?? Icons.label_outline;
}
