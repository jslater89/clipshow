...
Clipshow targets desktop Windows and Linux as built for this project. Smooth playback depends on your GPU and drivers; in practice the player stack favors hardware-backed decoding where it is available (FVP / libmdk). Adjust decoder-related options under Workspace settings if you need to tune behavior on your machine.
...
For the integration to make sense visually, OBS should expose program scenes you can map in Clipshow: one for your live look when you are *not* playing a clip (**Face**), and one where this application’s window is captured for **clip playout** (**Video**). For **OSG Mode**, the recommended layout is a dedicated **home scene** (for example “Clipshow OSG”) that contains a Window Capture of Clipshow; nest that scene on every live look and leave the nest items enabled. In Workspace settings (§11.3), set **OSG Overlay Scene** to the home scene and **OSG Overlay Source** to the capture inside it—Clipshow toggles that source in the home scene so graphics stay available across program cuts. Leave the overlay scene blank to use the older model (toggle a same-named source on the current program scene only). You choose the Video/Face scene names and overlay fields on the **Scene Switch** tab; defaults for the program scenes are “Face Scene” and “Video Scene.”
...
Create tag sets, attach tags (with optional semantic types), and assign up to five **quick slots** (keys **1–5** while in OSG Mode). Type in a slot to filter by tag set name (same autocomplete style as **Add Tag**), pick a suggestion to bind it, or clear the field / use the clear control to leave the slot empty. Enable **OSG Mode** on the **On-Screen Graphics** tab in Workspace settings (§11.2). **Enter OSG Mode** switches the app to a transparent full-window graphics surface. When OBS integration is enabled and an **OSG Overlay Source** is set, Clipshow resolves that source inside the configured **OSG Overlay Scene** when one is set, or on the **current** OBS program scene when the overlay scene is blank; enables it after the first frame paints; and disables the same item on **Escape**—program scene is not switched. If the source is missing from the target scene, enter fails with an error. Leave the overlay source empty to skip OBS overlay automation (webhooks may still fire). Hotkeys **6–0** toggle OSG presets; **1–5** switch among quick-slot tag sets.

Recommended OBS setup: put the Clipshow Window Capture in a home scene (for example “Clipshow OSG”), nest that scene on every live look, set **OSG Overlay Scene** / **OSG Overlay Source** accordingly, and leave the nested scene items enabled so you can cut program while graphics stay available.
...
**OBS** — WebSocket address, port, password, and **program** scene names for **video** (clip playout), **face** (return after playout), optionally **OSG Overlay Scene** and **OSG Overlay Source** (home-scene targeting when the scene is set; otherwise the source is enabled on the current program scene during OSG Mode—§7), and optionally **capture** (switched before OBS recording starts in Capture mode—§8). An empty overlay source disables OSG overlay automation. **Save OBS** applies that block.
