...
* **Dashboard layout:** Header plus **left** file list (**Reveal On Filesystem** per master/clip row via icon or context menu; workspace folder reveal in the header) and **right** tab column (Manage, Capture, Bake Queue when recipes exist, Tag Sets); preview vs tagging split; capture tab; bake queue panel.
