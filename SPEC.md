...
* **Video decoding:** Playout and Manage preview use **`fvp`** (`video_player` + libmdk) behind a `ClipMediaPlayer` adapter (`ClipPlayerView` owns clip bounds / seek / EOS policy). A **media_kit** adapter remains in-tree for compositor experiments but is not user-selectable. FVP decoder profiles and MDK/fvp verbosity are configurable per workspace.
...
The application expects a practical OBS scene layout; **Video** and **Face** program scene names are configurable in workspace settings (defaults below). OSG Mode optionally enables a **named source** either inside a fixed **home scene** or on whatever scene is currently program.
...
* **OSG overlay (optional):** A **source name** (typically Window Capture of the Clipshow window) and an optional **home scene** name. When the home scene is set, Clipshow enables/disables that source **inside the home scene** (`GetSceneItemId` + `SetSceneItemEnabled` on that scene)—nest the home scene on every live look and leave those nest items enabled so program cuts keep graphics. When the home scene is empty, behavior matches the earlier model: enable the source on the **current** program scene; enter fails if it is missing there. An empty source name disables overlay automation. Program scene is never switched for OSG Mode.
...
* **End of clip behavior:** At the configured **out** time (or end of file when no out point), playback **pauses** and the frame **holds** so the operator can keep drawing or talking over the image. After hard EOS (or the natural-end freeze), **seek** and **play** reopen a fresh decoder at the target position when the platform would otherwise ignore seeks—so transport scrubber position stays in sync (e.g. **Home** after completion).
* **Reversion sequence:**
...
* **Execution sequence:**
    1. If an **OSG overlay source** is configured (OBS profile enabled + non-empty source name), resolve that source: inside the configured **home scene** when set, otherwise on the **current** OBS program scene (`GetSceneItemId`, with `GetCurrentProgramScene` only in the latter case). If missing, abort enter with an operator-visible error (no OSG Mode UI).
...
* **Pipeline:** One-pass `ffmpeg`: seek/limit the master (`-ss`/`-t`), scale/letterbox to playout canvas, overlay OSG frames streamed as raw straight RGBA on stdin (`OsgFrameRenderer.renderFrameRawRgba`, dart:ui). Static hold/empty spans reuse the previous overlay buffer (visibility fingerprint) without re-rasterizing; enter/exit still render every frame. Encode once (H.264 + AAC) → copy to **playout output** folder (default `export`, same as Record playout) as `{displayName}_{sanitizedUserTags}_baked.mp4` (tags omitted when none; display name first so date-prefixed capture names stay prefix-sortable) with numeric dedup suffix. Temp work dir deleted afterward.
...
* **Hardware acceleration:** Playback uses **`fvp`** with configurable decoder profiles (exact codecs depend on OS/GPU; workspace settings expose ordering and diagnostics). At process start, **`fvp.registerWith`** also sets **`maxWidth` / `maxHeight`** (and **`fitMaxSize`**) from the restored workspace **Playout canvas size** (default **1920×1080**) so native textures are capped to the broadcast canvas rather than source resolution (e.g. 4K masters).
* **Playout canvas / window sizing:** In playout and OSG Mode, the OS window is sized and aspect-locked to workspace **Playout canvas size** (`PlayoutOutputSize`; default **1920×1080**). Any width×height is valid—not limited to 16:9. Decoded video uses **`BoxFit.contain`** (letterboxing/pillarboxing when source aspect differs). OSG and telestrator composite over the **full canvas** (normalized 0..1), including gutters—not only the fitted video rect. Canvas size also feeds the fvp texture cap above (applied at startup; restart after changing canvas for the new cap).
...
* **OSG Mode:** Tag Sets tab; transparent graphics surface; optional OBS overlay enable (home scene when set, else current program) + **`osg_on`** webhook on enter; disable that item + **`osg_off`** webhook on exit; tag-set quick slots **1–5**, preset toggles **6–0**.
...
* **Keyboard (preview and playout hotkey layers):** **`Space`** play/pause. **`Left` / `Right`** seek **±1 s**. **`Shift` + arrows** seek **±15 s**. **`Ctrl` + arrows** seek **±5 s**. **`Alt` + arrows** seek **±100 ms**. **`Home` / `End`** seek to clip start/end where implemented. **`↑` / `↓`** volume **±10%**; **`M`** mute toggle. In **playout**, preview, and **OSG Mode**, **`6`–`0`** toggle OSG presets **1–5**; in **OSG Mode**, **`1`–`5`** select tag-set quick slots. Manage preview hotkeys require the preview focus node; leaving tag/annotation fields in the tagging panel restores that focus, and pointer-down on the preview surface requests it (so click-to-play on the video also re-arms shortcuts).
