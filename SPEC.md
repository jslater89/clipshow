...
* **Video decoding:** Playout and Manage preview use a selectable **player backend** (`player.backend`): **`fvp`** (default; `video_player` + libmdk) or **`media_kit`** (libmpv). Shared clip bounds / seek / EOS policy live in `ClipPlayerView` over a `ClipMediaPlayer` adapter. FVP decoder profiles and MDK/fvp verbosity remain configurable per workspace and apply only when the FVP backend is active.
...
* **Hardware acceleration:** Default playback uses **`fvp`** with configurable decoder profiles (exact codecs depend on OS/GPU; workspace settings expose ordering and diagnostics). At process start, when the backend is FVP, **`fvp.registerWith`** also sets **`maxWidth` / `maxHeight`** (and **`fitMaxSize`**) from the restored workspace **Playout canvas size** (default **1920×1080**) so native textures are capped to the broadcast canvas rather than source resolution (e.g. 4K masters). **`media_kit`** uses libmpv’s own HW path; FVP decoder/texture-cap settings do not apply to it. Backend is chosen via workspace **`player.backend`** and applied at process start (restart required).
* **Playout canvas / window sizing:** In playout and OSG Mode, the OS window is sized and aspect-locked to workspace **Playout canvas size** (`PlayoutOutputSize`; default **1920×1080**). Any width×height is valid—not limited to 16:9. Decoded video uses **`BoxFit.contain`** (letterboxing/pillarboxing when source aspect differs). OSG and telestrator composite over the **full canvas** (normalized 0..1), including gutters—not only the fitted video rect. Canvas size also feeds the fvp texture cap above when using the FVP backend (applied at startup; restart after changing canvas for the new cap).
